...
- Fixed state class validation for energy sensors and update DC Charger capacity sensors to correct state class (#238)
...
- Added automatic restart of the main loop (when not running in Docker) if the health check fails (Docker containers are automatically restarted by the Docker daemon using the health check of the container)
