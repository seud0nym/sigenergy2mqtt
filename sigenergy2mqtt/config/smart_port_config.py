        ...
        if isinstance(config, dict):
            for field, value in config.items():
                ...
                match field:
                    ...
                    case "username":
                        self.username = cast(str, check_string(value, f"modbus.smart-port.module.{field}"))
                    case "password":
                        self.password = cast(str, check_string(value, f"modbus.smart-port.module.{field}"))
                    case "pv-power":
                        self.pv_power = cast(str, check_string(value, f"modbus.smart-port.module.{field}"))
                    case _:
...
class SmartPortConfiguration:
        ...
        if isinstance(config, dict):
            for field, value in config.items():
                match field:
                    ...
                    case "mqtt":
                        self.mqtt = self.configure_mqtt(value, override)
                    case _:
    ...
    def configure_mqtt(self, topics: list, override: bool = False) -> list[TopicConfig]:
            ...
            for config in topics:
                if isinstance(config, dict):
                    ...
                    for field, value in config.items():
                        ...
                        match field:
                            case "topic":
                                topic.topic = cast(str, check_string(value, f"modbus.smart-port.mqtt.{field}", allow_none=True, allow_empty=False))
                            case "gain":
                                topic.gain = cast(int, check_int(value, f"modbus.smart-port.mqtt.{field}", allow_none=False, min=1))
                            case _:
