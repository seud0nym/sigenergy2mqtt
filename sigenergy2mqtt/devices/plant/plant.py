...
class PowerPlant(ModbusDevice):
    ...
    async def _register_sensors(self, firmware: str, output_type: int, power_phases: int, modbus_client: ModbusClient) -> None:
        ...
        self._add_sensor(self._plant_pv_power, self._consumption_group)
        ...
        if self.has_battery:
            ...
            self._add_sensor(battery_power, self._consumption_group)
            ...
            self._add_sensor(derived.BatteryChargingPower(self.plant_index, battery_power), battery_power)
            self._add_sensor(derived.BatteryDischargingPower(self.plant_index, battery_power), battery_power)
            self._add_sensor(derived.PlantDailyChargeEnergy(self.plant_index, total_charge_energy), total_charge_energy, search_children=False)
            self._add_sensor(derived.PlantDailyDischargeEnergy(self.plant_index, total_discharge_energy), total_discharge_energy, search_children=False)
        ...
        self._add_sensor(self._total_pv_power, self._plant_pv_power, search_children=False)
        if self._smartport:
            ...
            if smartport_pv_power and not smartport_pv_power.isspace():
                for sensor in self._smartport.sensors.values():
                    if sensor.__class__.__name__ == smartport_pv_power:
                        ...
                        self._add_sensor(self._total_pv_power, sensor, search_children=True)
                        break
            if self._plant_3rd_party_pv_power:
                self._add_sensor(self._plant_3rd_party_pv_power)
                self._add_sensor(self._total_pv_power, self._plant_3rd_party_pv_power, search_children=False)
            ...
            if self._plant_3rd_party_pv_power:
                self._add_sensor(self._plant_3rd_party_pv_power, self._consumption_group)
                self._add_sensor(self._total_pv_power, self._plant_3rd_party_pv_power, search_children=False)
        ...
        match plant_consumed_power.method:
            case ConsumptionMethod.CALCULATED:
                ...
                self._add_sensor(plant_consumed_power, self._total_pv_power, battery_power, active_power, grid_status, search_children=True)
            case ConsumptionMethod.GENERAL:
                self._add_sensor(plant_consumed_power, general_load_power)
            case ConsumptionMethod.TOTAL:
                self._add_sensor(plant_consumed_power, total_load_power)
        ...
        self._add_sensor(plant_lifetime_pv_energy, "Lifetime Production")
        self._add_sensor(plant_3rd_party_lifetime_pv_energy, "Lifetime Production")
        self._add_sensor(total_lifetime_pv_energy, plant_lifetime_pv_energy, plant_3rd_party_lifetime_pv_energy)
        self._add_sensor(derived.PlantDailyPVEnergy(self.plant_index, plant_lifetime_pv_energy), plant_lifetime_pv_energy)
        self._add_sensor(derived.TotalDailyPVEnergy(self.plant_index, total_lifetime_pv_energy), total_lifetime_pv_energy)
