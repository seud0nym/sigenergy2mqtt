...
class PowerPlant(ModbusDevice):
    ...
    async def _register_sensors(self, firmware: str, output_type: int, power_phases: int, modbus_client: ModbusClient) -> None:
        ...
        if self.has_battery:
            ...
            soc = ro.PlantBatterySoC(self.plant_index)
            charge_cutoff_soc = ro.ChargeCutOffSoC(self.plant_index)
            rte = derived.PlantBatteryRTE(self.plant_index, soc, charge_cutoff_soc, total_charge_energy, total_discharge_energy)
            self._add_read_sensor(soc)
            ...
            self._add_read_sensor(charge_cutoff_soc)
            ...
            self._add_derived_sensor(rte, soc, charge_cutoff_soc, total_charge_energy, total_discharge_energy, search_children=False)
            self._add_derived_sensor(derived.PlantLifetimeBatteryRTE(self.plant_index), rte, search_children=False)
