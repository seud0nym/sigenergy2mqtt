...
class PowerPlant(ModbusDevice):
    ...
    async def _register_sensors(
        ...
        if self.has_battery:
            ...
            current_soc = ro.PlantBatterySoC(self.plant_index)
            charge_soc = rw.ESSChargeCutOffSOC(self.plant_index)
            discharge_soc = rw.ESSDischargeCutOffSOC(self.plant_index)
            backup_soc = rw.ESSBackupSOC(self.plant_index)
            self._add_sensor(current_soc)
            ...
            self._add_sensor(backup_soc)
            self._add_sensor(charge_soc)
            self._add_sensor(discharge_soc)
