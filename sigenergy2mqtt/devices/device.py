...
class Device(dict[str, str | list[str]], metaclass=abc.ABCMeta):
    ...
    def _create_sensor_scan_groups(self) -> dict[str, list[ReadableSensorMixin]]:
        ...
        for sensor in sorted(
            ...
            key=lambda s: (s.scan_interval, s.device_address, s.address),
        ):
        ...
        # Create sensor scan groups for remaining readable sensors
        group_name: str | None = None
        for sensor in sorted(
    ...
    async def publish_updates(self, modbus_client: ModbusClient, mqtt_client: mqtt.Client, name: str, *sensors: Sensor) -> None:
        ...
        if multiple:
            first_address = min([s.address for s in sensors if isinstance(s, ModbusSensorMixin) and s.publishable])
            last_address = max([s.address + s.count - 1 for s in sensors if isinstance(s, ModbusSensorMixin) and s.publishable])
            count = last_address - first_address + 1
            device_address, input_type = next((s.device_address, s.input_type) for s in sensors if isinstance(s, ModbusSensorMixin) and s.address == first_address)
        elif isinstance(sensors[0], ModbusSensorMixin):
            count = sensors[0].count
            first_address = sensors[0].address
            last_address = first_address + count - 1
            device_address = sensors[0].device_address
            input_type = sensors[0].input_type
        interval: int = min([s.scan_interval for s in sensors if isinstance(s, ReadableSensorMixin)])
        ...
        while self.online:
            ...
            forced = [sensor.__class__.__name__ for sensor in sensors if sensor.force_publish]
            if any(forced):
                if debug_logging:
                    logging.debug(f"{self.name} Sensor Scan Group [{name}] wait interrupted with {next_publish - now:.2f}s remaining because force_publish set on {forced}")
            ...
            if next_publish <= now:
                ...
                try:
                    async with lock.lock():
                        ...
                        if elapsed > interval and modbus_client.connected:
