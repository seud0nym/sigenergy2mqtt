...
| `SIGENERGY2MQTT_MODBUS_AUTO_DISCOVERY_EXCLUDE` | Comma-delimited list of Device class names to be excluded from auto-discovery. Defaults to "PID,PSS". Valid class names are "ACCharger", "DCCharger", "PID", "PSS". You cannot exclude Plant or Inverter. [<sup>(More…)</sup>](README.md#opt_modbus_auto_discovery_exclude) | 2026.6.5 |
...
| `SIGENERGY2MQTT_MODBUS_AUTO_DISCOVERY_TIMEOUT` | The Modbus timeout, in seconds, to use when performing auto-discovery of Sigenergy devices on the network. The default is `0.5` seconds. [<sup>(More…)</sup>](README.md#opt_modbus_auto_discovery_timeout) | 2025.5.12 |
...
| `SIGENERGY2MQTT_MODBUS_AUTO_DISCOVERY_NETWORKS` | A comma-separated list of IPv4 networks in CIDR notation to scan during auto-discovery (e.g. `192.168.1.0/24,10.0.0.0/24`). Use this to scan networks accessible via routing but not directly attached to a network interface. [<sup>(More…)</sup>](README.md#opt_modbus_auto_discovery_networks) | 2026.5.15 |
| `SIGENERGY2MQTT_MODBUS_AUTO_DISCOVERY_MAX_DEVICE_ID` | The maximum device ID to use when performing auto-discovery of Sigenergy devices on the network. The default is `10`, and it must be between 1 and 246 (inclusive). [<sup>(More…)</sup>](README.md#opt_modbus_auto_discovery_max_device_id) | 2026.6.13 |
...
| `SIGENERGY2MQTT_MODBUS_PID_DEVICE_ID` | The Sigenergy PID Device ID. May be specified as a comma-separated list. [<sup>(More…)</sup>](README.md#opt_modbus_pid_device_id) | 2026.6.4 |
| `SIGENERGY2MQTT_MODBUS_PSS_DEVICE_ID` | The Sigenergy PSS Device ID. May be specified as a comma-separated list. [<sup>(More…)</sup>](README.md#opt_modbus_pss_device_id) | 2026.6.4 |
...
| `SIGENERGY2MQTT_MODBUS_LOG_SKIPPED` | If `false`, Modbus errors like "ERROR: request ask for transaction_id=x but got id=y, Skipping" and "ERROR: request ask for id=x but got id=y, Skipping" will NOT be logged. The default is `false`. [<sup>(More…)</sup>](README.md#opt_modbus_log_skipped) | 2025.6.12 |
...
| `SIGENERGY2MQTT_PERSISTENCE_DEBUG` | Set to `true` to enable debug logging for the state persistence system. (default: `false`) [<sup>(More…)</sup>](README.md#opt_persistence_debug) | 2026.6.8 |
...
<table>
  ...
  <tbody>
    ...
    <tr><td><code>SIGENERGY2MQTT_PVOUTPUT_UPLOAD_LOG_LEVEL</code></td><td>Set the PVOutput upload payload log level. Valid values are: <code>DEBUG</code>, <code>INFO</code>, <code>WARNING</code>, <code>ERROR</code> or <code>CRITICAL</code>. Default is <code>INFO</code>. <a href="README.md#opt_pvoutput_upload_log_level"><sup>(More…)</sup></a><td>2026.6.16</td><tr>
...
| Name | Description | Since |
|------|-------------|-------|
| `SIGENERGY2MQTT_DIAGNOSTICS_ENABLED` | Set to `false` to disable the diagnostics web server. Note: If running  within Docker, this flag is ignored and treated as true. [<sup>(More…)</sup>](README.md#opt_diagnostics_enabled) | 2026.7.29 |
| `SIGENERGY2MQTT_DIAGNOSTICS_HOST` | The address to which the diagnostics web server is to be bound. (default: `0.0.0.0`) [<sup>(More…)</sup>](README.md#opt_diagnostics_host) | 2026.7.29 |
| `SIGENERGY2MQTT_DIAGNOSTICS_PORT` | The diagnostics web server listening port. (default: `8502`) [<sup>(More…)</sup>](README.md#opt_diagnostics_port) | 2026.7.29 |
| `SIGENERGY2MQTT_DIAGNOSTICS_REFRESH_INTERVAL` | The diagnostics web page refresh interval in seconds. Must be greater than `0`. (default: `5.0`) [<sup>(More…)</sup>](README.md#opt_diagnostics_refresh_interval) | 2026.7.29 |
