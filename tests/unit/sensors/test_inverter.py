...
class TestInverterDerived:
    def test_battery_charging_discharging_power(self, mock_config):
        ...
        charging.update_from_source_sensor(source)
        discharging.update_from_source_sensor(source)
    ...
    async def test_pv_string_power(self, mock_config):
        ...
        assert "plant=0,dev=1,string=1" in p.amperes.name
        assert "plant=0,dev=1,string=1" in p.volts.name

        # Set some values
        v.set_state(4000)  # 400.0V (gain=10)
        c.set_state(100)  # 1.0A (gain=100)

        p.update_from_source_sensor(v)
        assert p.volts.value == 400.0
        assert p.amperes.value is None

        p.update_from_source_sensor(c)
