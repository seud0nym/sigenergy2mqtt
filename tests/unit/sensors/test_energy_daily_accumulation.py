...
class TestEnergyDailyAccumulationSensor:
    ...
    def test_on_added_to_device_loads_midnight_state(self, source_sensor, mock_state_store):
        ...
        with patch.dict(Sensor._used_unique_ids, clear=True), patch.dict(Sensor._used_object_ids, clear=True):
            ...
            mock_state_store.load_sync.assert_any_call(
                Category.SENSOR, "sigen_test_source_uid.atmidnight", stale_after=timedelta(hours=24), debug=sensor.debug_logging
            )
    ...
    def test_load_midnight_state_invalid(self, source_sensor, mock_state_store):
        ...
        with patch.dict(Sensor._used_unique_ids, clear=True), patch.dict(Sensor._used_object_ids, clear=True):
            ...
            assert sensor._state_at_midnight is None
            mock_state_store.delete_sync.assert_called_with(Category.SENSOR, "sigen_test_source_uid.atmidnight", debug=sensor.debug_logging)
    ...
    async def test_update_state_at_midnight(self, source_sensor, mock_state_store):
        with patch.dict(Sensor._used_unique_ids, clear=True), patch.dict(Sensor._used_object_ids, clear=True):
            ...
            mock_state_store.save_sync.assert_called_with(
                Category.SENSOR, "sigen_test_source_uid.atmidnight", "150.0", debug=sensor.debug_logging
            )
