...
from sigenergy2mqtt.sensors.base import Sensor, ReadOnlySensor, ResettableAccumulationSensor, AlarmSensor, AlarmCombinedSensor  # noqa: E402
    ...
    def test_resettable_accumulation_negative_increase(self, tmp_path):
        with patch("sigenergy2mqtt.sensors.base.Config.persistent_state_path", str(tmp_path)):
            ...
            with patch.dict(Sensor._used_unique_ids, clear=True), patch.dict(Sensor._used_object_ids, clear=True):
                sensor = ResettableAccumulationSensor("Accum", "sigenergy_acc", "sigenergy_acc", source, ModbusClient.DATATYPE.UINT32, "kWh", DeviceClass.ENERGY, StateClass.TOTAL, "mdi:energy", 1.0, 2)
...
class TestAlarmSensors:
    def test_alarm_sensor_binary(self):
        with patch.dict(Sensor._used_unique_ids, clear=True), patch.dict(Sensor._used_object_ids, clear=True):
            sensor = AlarmSensor("Alarm", "sigenergy_alarm", 0, 1, 30001, Protocol.V2_4, "Equipment")
        ...
        with patch.dict(Sensor._used_unique_ids, clear=True), patch.dict(Sensor._used_object_ids, clear=True):
            ...
            class ProxyAlarm(AlarmSensor):
                ...
                async def get_state(self, **kwargs):
                    raw = self.latest_raw_state
                    return "No Alarm" if raw == 0 else "Error"
