...
def test_check_modbus_no_clients(monkeypatch):
    ...
    svc = MonitorService([])
    assert svc._check_modbus()[0] is False
...
def test_check_modbus_healthy(monkeypatch):
    ...
    svc = MonitorService([])
    assert svc._check_modbus()[0] is True


def test_check_mqtt_no_snapshot(monkeypatch):
    monkeypatch.setattr("sigenergy2mqtt.mqtt.mqtt_health_registry.snapshot", dict, raising=False)
    svc = MonitorService([])
    fake_client = FakeMqttClient()
    assert svc._check_mqtt(fake_client)[0] is False
...
def test_check_mqtt_healthy(monkeypatch, fake_mqtt_client):
    ...
    svc = MonitorService([])
    assert svc._check_mqtt(fake_mqtt_client)[0] is True
...
def test_clean_removes_topics(monkeypatch, tmp_path):
...
def test_publish_health_writes_mqtt(monkeypatch, fake_mqtt_client, tmp_path):
    svc = MonitorService([])
