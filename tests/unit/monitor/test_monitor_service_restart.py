...
async def test_restart_triggered_after_retries_exceeded(tmp_path):
    ...
    with patch("sigenergy2mqtt.main.restart.restart_controller") as mock_rc, patch("sigenergy2mqtt.config.active_config.health_check") as mock_hc:
        mock_hc.enabled = True
        mock_hc.timeout = 5
        mock_hc.retries = 2
        mock_hc.start_period = 0
...
async def test_failure_count_resets_on_healthy(tmp_path):
    ...
    with patch("sigenergy2mqtt.main.restart.restart_controller"), patch("sigenergy2mqtt.config.active_config.health_check") as mock_hc:
        mock_hc.enabled = True
        mock_hc.timeout = 5
        mock_hc.retries = 10
        mock_hc.start_period = 0
