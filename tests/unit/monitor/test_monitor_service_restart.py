...
def _make_svc(tmp_path: Path) -> "tuple[MonitorService, FakeMqttClient]":
    ...
    svc._check_modbus = MagicMock(return_value=(True, 1))
    svc._check_mqtt = MagicMock(return_value=(True, 1))
...
async def test_publish_health_skipped_when_disabled_non_docker(tmp_path):
    ...
    with patch("sigenergy2mqtt.config.active_config.health_check") as mock_hc, patch("sigenergy2mqtt.monitor.service.is_docker", return_value=False):
        mock_hc.enabled = False
...
async def test_publish_health_forced_when_docker_even_if_disabled(tmp_path):
    ...
    svc, mqtt_client = _make_svc(tmp_path)
    with patch("sigenergy2mqtt.config.active_config.health_check") as mock_hc, patch("sigenergy2mqtt.monitor.service.is_docker", return_value=True):
        mock_hc.enabled = False
        mock_hc.timeout = 5
        mock_hc.retries = 3
...
async def test_no_restart_in_docker_even_after_retries(tmp_path):
    ...
    svc, mqtt_client = _make_svc(tmp_path)
    svc._check_modbus = MagicMock(return_value=(False, 1))
...
async def test_restart_triggered_after_retries_exceeded(tmp_path):
    ...
    with patch("sigenergy2mqtt.main.restart.restart_controller") as mock_rc, patch("sigenergy2mqtt.config.active_config.health_check") as mock_hc, patch("sigenergy2mqtt.monitor.service.is_docker", return_value=False):
        mock_hc.enabled = True
        mock_hc.timeout = 5
        mock_hc.retries = 2
        mock_hc.start_period = 0
...
async def test_failure_count_resets_on_healthy(tmp_path):
    ...
    with patch("sigenergy2mqtt.main.restart.restart_controller"), patch("sigenergy2mqtt.config.active_config.health_check") as mock_hc, patch("sigenergy2mqtt.monitor.service.is_docker", return_value=False):
        mock_hc.enabled = True
        mock_hc.timeout = 5
        mock_hc.retries = 10
        mock_hc.start_period = 0
