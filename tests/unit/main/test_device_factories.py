...
async def test_make_plant_and_inverter_missing_tz_offset():
    ...
    def mock_get_state_side_effect(*args, **kwargs):
        if "InverterSerialNumber" in str(args[0].__class__):
            return "SN1"
        if "InverterModel" in str(args[0].__class__):
            return "MDL1"
        if "PACKBCUCount" in str(args[0].__class__):
            return 1
        if "SystemTimeZone" in str(args[0].__class__):
            return None
        if "InverterFirmwareVersion" in str(args[0].__class__):
            return "V122R001C00SPC112B701P"
        if "OutputType" in str(args[0].__class__):
            return 1
        if "ESSPreHeatingEnable" in str(args[0].__class__):
            return 1
        return None
    ...
    with (
        ...
        assert mock_plant_create.call_args[0][4] == UTC
